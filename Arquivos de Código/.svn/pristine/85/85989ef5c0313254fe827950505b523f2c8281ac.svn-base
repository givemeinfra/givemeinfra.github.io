...
public class MatrixSideItem extends Figure implements INodeListener, MouseListener {
	
	private Node node;
	private int index;
	
	public MatrixSideItem(Node node, int index) {
		
		this.node = node;
		this.index = index;
		node.addListener(this);
		node.setProperty(Properties.MATRIX_SIDEITEM, this);
	...
	protected void paintFigure(Graphics graphics) {
		...
		graphics.drawString(index + "  " + node.getID(), 5, getBounds().height/2 - 7);
	...
	public void dispose() {
		...
		node.setProperty(Properties.MATRIX_SIDEITEM, null);
