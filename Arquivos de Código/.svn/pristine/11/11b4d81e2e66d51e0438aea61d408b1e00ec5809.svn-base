...
public class ActivityScreen implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		...
		if(LoginControl.GET_INSTANCE().logado==true){
			...
			if (LoginControl.GET_INSTANCE().currentDeveloper.getCoordination()) {
				Shell shell = this.window.getShell();
				WizardDialog wd = new WizardDialog(shell, new TemplateWizard("atividade"));
				wd.setHelpAvailable(false);
				wd.setPageSize(200, 260);
				wd.open();
			} else {
				Shell shell = this.window.getShell();
